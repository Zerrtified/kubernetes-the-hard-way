...
Generate a kubeconfig file for the kube-node0 worker node:
...
```text
kube-node0.kubeconfig
kube-node1.kubeconfig
```
...
Copy the `kubelet` and `kube-proxy` kubeconfig files to the kube-node0 instance:

```bash
for host in kube-node0 kube-node1; do
  ssh steve@$host "mkdir /var/lib/{kube-proxy,kubelet}"
  
  scp kube-proxy.kubeconfig \
    steve@$host:/var/lib/kube-proxy/kubeconfig \
  
  scp ${host}.kubeconfig \
    steve@$host:/var/lib/kubelet/kubeconfig
done
```
